...
分析風格：先結論再展開、每個判斷引用具體數字、用全球案例對比。
語言規則：全程使用繁體中文，絕對不使用任何英文單字。brand trust→品牌信任、occupancy rate→入住率、IRR→年化報酬率、LTV→終身價值、CAC→獲客成本、DCOH→現金儲備天數。所有專業術語必須翻譯成中文。
...
def render_ai_analysis(params, mc=None, stress=None):
    ...
    if run_btn:
        ...
        # Stream response with tip rotation above
        # 用 st.status 同時顯示 tip 和串流，避免 empty() 衝突
        _tip_idx = [0]
        _last_switch = [time.time()]
        TIP_INTERVAL = 8

        # Tip 顯示在上方
        def _update_ai_tip(idx):
            tid, title, body = ai_tips[idx % len(ai_tips)]
            import re
            body_clean = re.sub(r'\*\*(.+?)\*\*', r'\1', body)
            ai_tip_box.info(f"**{title}**\n\n{body_clean}")
...
def _build_opus_prompt(params, metrics, comparison, r, stress):
    ...
    lines.append("""
請提供完整、詳盡的顧問分析報告，涵蓋七部分。每部分都要深入展開，不要只寫一段話就跳下一部分。整份報告至少 3,000 字。

一、整體判斷（能不能做？一段話結論 + 三個關鍵數字佐證 + 與全球案例對比定位）

二、因果診斷（從瀑布圖追根因，用因果鏈 A→B→C。至少分析 3 條因果鏈。每條鏈附上具體數字。找出「改一個就能扭轉全局」的關鍵節點。）

三、組合效應（逐一解讀每組對比模擬結果，說明差異的根因。哪些配套有超線性增強（1+1>2）？缺什麼最致命？用數字量化每個配套的邊際貢獻。）

四、配套行動方案（分三層：
   第一層「必須同步」：列出 3-5 個必做項目，每個附上預期改善幅度
   第二層「增強」：列出 2-3 個加分項目
   第三層「選擇題」：列出需要取捨的決策點，附上兩邊的利弊分析
   每個行動項目都要具體到「做什麼、花多少、預期效果多少」）

五、風險地圖（結合規則觸發狀態和壓力測試結果，畫出風險優先矩陣。對每個高風險項目，引用一個全球案例說明後果。提出每個風險的具體緩解措施。）

六、如果你是決策者（站在決策者的角度，建議優先做什麼？按時間排序：前 6 個月 / 6-12 個月 / 1-3 年。每個時間段 3-5 個具體行動，附上數字支撐和優先順序理由。）

七、觸點設計機會掃描（用人性影響矩陣，找出 5 個高槓桿觸點設計機會。每個機會說明：觸發什麼人性反應、對應哪個財務角色、在用戶旅程哪個階段、預期對模擬器哪個參數有影響、建議怎麼調整參數來驗證。）

所有分析必須引用具體數字，不要空泛。全程使用繁體中文，不使用英文。格式要求：使用 ### 作為最高級標題，不要用 # 或 ##。""")
